            ...
            for x in range(0, size):
                if _USE_NUMPY:
                    ...
                    for y in range(render_attrs.y_start, render_attrs.y_end):
                        ...
                        ray_L_XY = np.array(
                            [
                                x * render_attrs.pixel_size,
                                y * render_attrs.pixel_size,
                                0,
                            ]
                            - light_point
                        )
                        ...
                        RD = np.dot(
                            ray_L_XY, ray_L_XY
                        )  # square # np.linalg.norm(ray_L_XY)
                        L_dir = interpolation(
                            beta
                    ...
                    for y in range(render_attrs.y_start, render_attrs.y_end):
                        ...
                        beta = np.degrees(
                            np.arccos((y_ * render_attrs.pixel_size) / RD)
                        )
                        L_dir = interpolation(
                            beta
                        )  # candela value for this ray (vertical angle)
